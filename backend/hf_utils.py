...
Dosya adı neden ``hf_utils``: Hermes'in kaynak ağacında da üst düzey bir
``utils`` modülü var ve APK'da ikisi aynı Python yolunda duruyor. Aynı adı
kullansaydık hangisinin yükleneceği içe aktarma sırasına bağlı kalırdı —
Hermes kendi ``utils``i yerine bizimkini alıp kırılırdı.
"""
